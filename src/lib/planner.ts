...
async function runWebLLMPlan(
  ...
  const parsed = parseModelPlan(raw, request);
...
function parseModelPlan(raw: string, request: PlanRequest): PlannedCommand {
  ...
  const explanation =
    Array.isArray(value) || !value.explanation
      ? "Generated by the local planner with FFmpeg documentation context."
      : value.explanation;
  const extension = inferOutputExtension(request.prompt, explanation, args);
  const ensured = ensureCommandOutput(args, request.file, extension);
  return {
    args: ensured,
    explanation,
